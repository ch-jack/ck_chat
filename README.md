...
> 本仓库只发布构建后的 `ck_chat.zip` 和使用文档，不展开上传源码文件。安装时下载 `ck_chat.zip` 解压到服务器资源目录即可。
...
- GitHub Actions 自动校验并发布 `ck_chat.zip`。
...
配置文件: 解压 `ck_chat.zip` 后的 `config.lua`
...
- 检查 `ck_chat.zip` 是否存在且非空
- 输出 `ck_chat.zip` 的 SHA256
- 上传 `ck_chat.zip` 作为构建产物
